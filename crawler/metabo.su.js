...
module.exports = (async function(array) {
    ...
    try {
        ...
        await page.goto("https://metabo.su",{timeout: 300000});
        ...
        if (array[i] != "") {
            ...
            try {
                ...
                let price = $('[itemprop="offers"]').find('[itemprop="price"]').attr('content');
                    price = price ? parseInt( price.replace(/[^.\d]+/g,"").replace( /^([^\.]*\.)|\./g, '$1' ) ):false;

                // Перечеркнутая цена
                let priceAdd =  (  $('.product-page').find('.compare-at-price').text()  ).toString().replace(/[^.\d]+/g,"").replace( /^([^\.]*\.)|\./g, '$1' );
                    priceAdd = priceAdd && !isNaN(priceAdd) ? parseInt(priceAdd) : false;
               

                // Парсинг наличия
                let selector = $('.product-page').find('.stocks').text().toLowerCase().replace(/\s+/g,'');
                let avalible = ( selector == "вналичии" ) ? true : false;
