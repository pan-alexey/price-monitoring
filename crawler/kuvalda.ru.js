...
module.exports = (async function(array) {
    ...
    try {
        ...
        await page.goto("https://krasnodar.kuvalda.ru",{timeout: 300000});
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (array[i] != "") {
            ...
            try {
                ...
                //Цена на товар
                let price =  ( $('#product-data').find('.product-buy__price-value').text()  ).toString().replace(/[^.\d]+/g,"").replace( /^([^\.]*\.)|\./g, '$1' );
                    price = price && !isNaN(price) ? parseInt(price) : false;

                // Перечеркнутая цена
                let priceAdd =  ( $('#product-data').find('.product-buy__price-old').text()  ).toString().replace(/[^.\d]+/g,"").replace( /^([^\.]*\.)|\./g, '$1' );
                    priceAdd = priceAdd && !isNaN(priceAdd) ? parseInt(priceAdd) : false;
               
                // Парсинг наличия
                let selector = $("#product-data").find(".product-buy__state-title").text().toLowerCase();
