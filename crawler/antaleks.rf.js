    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (array[i] != "") {
            ...
            try {
                ...
                //Цена на товар
                let price = $('[itemtype="https://schema.org/Offer"]').find('[itemprop="price"]').attr('content');;
                    price = price && !isNaN(price) ? parseInt(price) : false;

                // Перечеркнутая цена
                let priceAdd =  false;
                // Парсинг наличия
                let selector = $('.good__price-wrp').find('[itemprop="availability"]')
                               .text().toLowerCase();
                let avalible = ( !( selector.indexOf("нет в наличии")>=0 ) ) ? true : false;
